...
import { CORPUS_REGULATIONS } from "./data/corpus-regulations.js";
...
// --- All regulations from Danish Law MCP corpus ---
// Generated by: python3 scripts/ingest-from-law-db.py
// Replaces the old hardcoded ensRegs, sikRegs, and expansion arrays.

const allRegs = CORPUS_REGULATIONS;
...
insertRegBatch();
const ensCount = allRegs.filter(r => r[0] === "energistyrelsen").length;
const sikCount = allRegs.filter(r => r[0] === "sikkerhedsstyrelsen").length;
const ftCount = allRegs.filter(r => r[0] === "forsyningstilsynet").length;
